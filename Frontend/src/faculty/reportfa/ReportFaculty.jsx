...
export default function ReportFaculty() {
  ...
  const fetchData = () => {
    axios
      .get(`http://localhost:3000/getFacultyReport/${user._id}`)
      .then((response) => {
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      const response = await axios.post("http://localhost:3000/FacultyReport", {
        ...formData,
        email: user.email,
        user: user._id,
      });
  ...
  const handleSave = async () => {
    try {
      const response = await axios.post(
        `http://localhost:3000/SaveFacultyReport/${user._id}`,
        {
          ...formData,
          email: user.email,
        }
